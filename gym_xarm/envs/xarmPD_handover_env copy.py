...
class XarmPDHandover(gym.GoalEnv):
    def __init__(self):
        ...
        # load lego
        lg_v = p.createVisualShape(shapeType=p.GEOM_BOX, halfExtents = [0.025, 0.1, 0.025], rgbaColor = [1,1,0,1])
        ...
        self.lego = p.createMultiBody(baseVisualShapeIndex=lg_v, baseCollisionShapeIndex = lg_c, baseMass = 0.1, basePosition=lego_pos, baseOrientation = p.getQuaternionFromEuler([0,0,self.np_random.uniform(-np.pi, np.pi)]))
        # load arm

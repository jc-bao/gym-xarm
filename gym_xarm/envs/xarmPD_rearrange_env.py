...
class XarmPDRearrangeEnv(gym.GoalEnv):
    def __init__(self):
        ...
        self.goal_space = spaces.Box(low=np.array([-0.35, -0.25]),high=np.array([0.35, 0.25]), dtype=np.float32)
        self.obj_space = spaces.Box(low=np.array([-0.35, -0.25]), high=np.array([0.35, 0.25]), dtype=np.float32)
        ...
        # load lego
        self.colors = [np.random.sample(size = 3).tolist() + [0.5] for _ in range(self.num_obj)]
        self.legos = [None] * self.num_obj
        for i in range(self.num_obj):
            lg = p.createVisualShape(shapeType=p.GEOM_BOX, halfExtents = [0.025]*3, rgbaColor = self.colors[i])
            lego_pos = np.concatenate((self.obj_space.sample(), [self.height_offset]))
            self.legos[i] = p.createMultiBody(baseVisualShapeIndex=lg, basePosition=lego_pos)
        # load arm
